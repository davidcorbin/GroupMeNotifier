...
class GroupMe:
    ...
    # Constructor
    def __init__(self):
        print "New Instance of GroupMe notifier"
    ...
    def auth(self):
        ...
        if not os.path.isfile(self.access_token_dir) or os.stat(self.access_token_dir).st_size == 0:
            ...
            token_file.close()
            print "token_access file written"
...
# Handler extending SimpleHTTPRequestHandler to capture localhost:8000 GET requests when server is started
class GetHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        if self.path.startswith('/?access_token'):
            ...
            self.send_header("Location", "http://daconex.me")
            self.end_headers()
        else:
            self.do_GET()
